...
class BasicSynchronousClient(object):
    ...
    def lidar_blueprint(self):
        ...
        depth_camera_bp = self.world.get_blueprint_library().find('sensor.lidar.ray_cast')
        depth_camera_bp.set_attribute('image_size_x', str(VIEW_WIDTH))
        depth_camera_bp.set_attribute('image_size_y', str(VIEW_HEIGHT))
        depth_camera_bp.set_attribute('fov', str(VIEW_FOV))
        return depth_camera_bp
    ...
    def setup_camera(self):
        ...
        depth_camera_transform = carla.Transform(carla.Location(x=-5.5, z=2.8), carla.Rotation(pitch=-15))
        ...
        self.depth_camera.listen(lambda image: weak_self().display_depths(weak_self, image))
    ...
    @staticmethod       
    def display_depths(weak_self, image) :
        image.convert(cc.LogarithmicDepth)
        depth = np.frombuffer(image.raw_data, dtype=np.dtype("uint8"))
        depth = np.reshape(depth, (image.height, image.width, -1))
        depth = depth[:, :, 0]
        image.save_to_disk("/home/a/Downloads/my_custom/LogarithmicDepth.png")
        print(depth, depth[image.height//2][image.width//2], depth.shape)
    ...
    def game_loop(self):
        ...
        try:
            ...
            self.client = carla.Client('192.168.0.22', 2000)
